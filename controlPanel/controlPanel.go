...
import (
	...
	"github.com/DCNT-developer/dcnt/controlPanel/files"
...
func ServeControlPanel(displayStateChannel chan state.DisplayState, statePointer *state.State, connections chan interface{}, controller *p2p.Controller, gitBuild string, nodeName string) {
	...
	indexTemplateData.Version = statePointer.GetdcntVersion()
	...
	controlPanelMux.HandleFunc("/dcnt", dcntHandler)
	controlPanelMux.HandleFunc("/dcntBatch", dcntBatchHandler)
...
func dcntBatchHandler(w http.ResponseWriter, r *http.Request) {
	if false == checkControlPanelPassword(w, r) {
		return
	}
	RequestData()
...
func dcntHandler(w http.ResponseWriter, r *http.Request) {
	defer func() {
		if r := recover(); r != nil {
			fmt.Println("Control Panel has encountered a panic in dcntHandler.\n", r)
		}
	}()
	if false == checkControlPanelPassword(w, r) {
		return
	...
	data := dcntQuery(item, value, false)
	w.Write([]byte(data))
}
...
func dcntQuery(item string, value string, batchQueried bool) []byte {
	if !batchQueried {
		RequestData()
	}
	switch item {
...
func checkControlPanelPassword(response http.ResponseWriter, request *http.Request) bool {
	if false == checkAuthHeader(request) {
		...
		response.Header().Add("WWW-Authenticate", `Basic realm="dcnt Control Panel"`)
...
func checkAuthHeader(r *http.Request) bool {
	if "" == StatePointer.GetRpcUser() {
		//no username was specified in the config file or command line, meaning dcnt control panel is open access
		return true
	}
