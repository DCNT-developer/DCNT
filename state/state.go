...
import (
	...
	"github.com/DCNT-developer/dcnt/common/constants/runstate"

	"github.com/DCNT-developer/dcnt/activations"
	...
	"github.com/DCNT-developer/dcnt/wsapi"
	"github.com/DCNT-developer/logrustash"

	"regexp"

	"github.com/DCNT-developer/dcnt/Utilities/CorrectChainHeads/correctChainHeads"
...
type State struct {
	...
	ConfigFilePath    string // $HOME/.factom/m2/dcnt.conf by default
	...
	dcntTLSEnable   bool
	dcntTLSKeyFile  string
	dcntTLSCertFile string
	dcntLocations   string
	...
	NetworkNumber int // Encoded into Directory Blocks(s.Cfg.(*util.dcntConfig)).String()
...
func (s *State) Clone(cloneNumber int) interfaces.IState {
	...
	configfile := fmt.Sprintf("%sdcnt%03d.conf", simConfigPath, cloneNumber)
	...
	newState.FactomNodeName = s.Prefix + "FNode" + number
	newState.dcntVersion = s.dcntVersion
	...
	newState.dcntTLSEnable = s.dcntTLSEnable
	newState.dcntTLSKeyFile = s.dcntTLSKeyFile
	newState.dcntTLSCertFile = s.dcntTLSCertFile
	newState.dcntLocations = s.dcntLocations
...
func (s *State) GetTlsInfo() (bool, string, string) {
	return s.dcntTLSEnable, s.dcntTLSKeyFile, s.dcntTLSCertFile
}

func (s *State) GetdcntLocations() string {
	return s.dcntLocations
}
...
func (s *State) LoadConfig(filename string, networkFlag string) {
	...
	if len(filename) > 0 {
		...
		// Get our dcnt configuration information.
		cfg := s.GetCfg().(*util.dcntConfig)
		...
		s.RpcUser = cfg.App.dcntRpcUser
		s.RpcPass = cfg.App.dcntRpcPass
		...
		s.dcntTLSEnable = cfg.App.dcntTlsEnabled
		if cfg.App.dcntTlsPrivateKey == "/full/path/to/dcntAPIpriv.key" {
			s.dcntTLSKeyFile = fmt.Sprint(cfg.App.HomeDir, "dcntAPIpriv.key")
		}
		if cfg.App.dcntTlsPublicCert == "/full/path/to/dcntAPIpub.cert" {
			s.dcntTLSCertFile = fmt.Sprint(cfg.App.HomeDir, "dcntAPIpub.cert")
		}
		externalIP := strings.Split(cfg.Walletd.dcntLocation, ":")[0]
		if externalIP != "localhost" {
			s.dcntLocations = externalIP
		}
		...
		s.MainSeedURL = "https://raw.githubusercontent.com/DCNT-developer/DCNT/master/seed/mainseed.txt"
		s.MainSpecialPeers = ""
		s.TestNetworkPort = "8109"
		s.TestSeedURL = "https://raw.githubusercontent.com/DCNT-developer/DCNT/master/seed/testseed.txt"
		s.TestSpecialPeers = ""
		s.LocalNetworkPort = "8110"
		s.LocalSeedURL = "https://raw.githubusercontent.com/DCNT-developer/DCNT/master/seed/localseed.txt"
		s.LocalSpecialPeers = ""
...
func (s *State) Init() {
	...
	default:
		panic("Bad value for Network in dcnt.conf")
	}
...
func (s *State) HookLogstash() error {
	hook, err := logrustash.NewAsyncHook("tcp", s.LogstashURL, "dcntLogs")
	if err != nil {
		return err
	}
...
func (s *State) GetdcntVersion() string {
	return s.dcntVersion
}
...
// future calls to Cfg().(s.Cfg.(*util.dcntConfig)).String()
...
func (s *State) GetNetworkBootStrapKey() (rval interfaces.IHash) {
	...
	case constants.NETWORK_CUSTOM:
		...
		if err != nil {
			panic(fmt.Sprintf("Cannot use a CUSTOM network without a CustomBootstrapKey specified in the dcnt.conf file. Err: %s", err.Error()))
		}
		return key
	}
...
func (s *State) GetNetworkBootStrapIdentity() (rval interfaces.IHash) {
	...
	case constants.NETWORK_CUSTOM:
		...
		if err != nil {
			panic(fmt.Sprintf("Cannot use a CUSTOM network without a CustomBootstrapIdentity specified in the dcnt.conf file. Err: %s", err.Error()))
		}
		return id
	}
	return primitives.NewZeroHash()
