...
func SavedcntState(state *State, d *DBState) (ss *SaveState) {
	state.LogPrintf("dbstateprocess", "SavedcntState(%d) called from %s", d.DirectoryBlock.GetHeader().GetDBHeight(), atomic.WhereAmIString(1))
	...
	state.LogPrintf("dbstateprocess", "SavedcntState(%d) saving  %d/%d authset", d.DirectoryBlock.GetHeader().GetDBHeight(), len(ss.FedServers), len(ss.AuditServers))
...
func (ss *SaveState) RestoredcntState(s *State) { //, d *DBState) {
	...
	s.MoveStateToHeight(ss.LLeaderHeight, ss.CurrentMinute) // RestoredcntState
