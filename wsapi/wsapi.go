...
import (
	...
	"github.com/DCNT-developer/btcutil/certs"
	...
	if Servers[state.GetPort()] == nil {
		...
		if tlsIsEnabled {
			...
			if !fileExists(tlsPrivate) && !fileExists(tlsPublic) {
				err := genCertPair(tlsPublic, tlsPrivate, state.GetdcntLocations())
				if err != nil {
...
func HandleProperties(ctx *web.Context) {
	...
	d := new(x)
	d.dcnt_Version = jsonResp.Result.(*PropertiesResponse).dcntVersion
...
func checkAuthHeader(state interfaces.IState, r *http.Request) error {
	if "" == state.GetRpcUser() {
		//no username was specified in the config file or command line, meaning dcnt API is open access
		return nil
	}
...
func checkHttpPasswordOkV1(state interfaces.IState, ctx *web.Context) bool {
	if err := checkAuthHeader(state, ctx.Request); err != nil {
		...
		ctx.ResponseWriter.Header().Add("WWW-Authenticate", `Basic realm="dcnt RPC"`)
