...
type dcntConfig struct {
	App struct {
		...
		P2POutgoing             int
		dcntTlsEnabled       bool
		dcntTlsPrivateKey    string
		dcntTlsPublicCert    string
		dcntRpcUser          string
		dcntRpcPass          string
		RequestTimeout          int
		RequestLimit            int
...
func (s *dcntConfig) String() string {
	...
	out.WriteString(fmt.Sprintf("\n    dcntTlsEnabled        %v", s.App.dcntTlsEnabled))
	out.WriteString(fmt.Sprintf("\n    dcntTlsPrivateKey     %v", s.App.dcntTlsPrivateKey))
	out.WriteString(fmt.Sprintf("\n    dcntTlsPublicCert     %v", s.App.dcntTlsPublicCert))
	out.WriteString(fmt.Sprintf("\n    dcntRpcUser          	%v", s.App.dcntRpcUser))
	out.WriteString(fmt.Sprintf("\n    dcntRpcPass          	%v", s.App.dcntRpcPass))
	...
	out.WriteString(fmt.Sprintf("\n    dcntLocation         %v", s.Walletd.dcntLocation))
...
func ConfigFilename() string {
	return GetHomeDir() + "/.factom/m2/dcnt.conf"
}

func GetConfigFilename(dir string) string {
	return GetHomeDir() + "/.factom/" + dir + "/dcnt.conf"
}
