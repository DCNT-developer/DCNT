...
func NetStart(s *state.State, p *FactomParams, listenToStdin bool) {
	...
	if p.dcntTLS == true {
		s.dcntTLSEnable = true
	}

	if p.dcntLocations != "" {
		if len(s.dcntLocations) > 0 {
			s.dcntLocations += ","
		}
		s.dcntLocations += p.dcntLocations
	}
