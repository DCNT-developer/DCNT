...
import (
	...
	"github.com/DCNT-developer/dcnt/elections"
...
func init() {
	...
	flag.StringVar(&p.ConfigPath, "config", "", "Override the config file location (dcnt.conf)")
	...
	flag.StringVar(&p.RpcUser, "rpcuser", "", "Username to protect dcnt local API with simple HTTP authentication")
	flag.StringVar(&p.RpcPassword, "rpcpass", "", "Password to protect dcnt local API. Ignored if rpcuser is blank")
	flag.BoolVar(&p.dcntTLS, "tls", false, "Set to true to require encrypted connections to dcnt API and Control Panel") //to get tls, run as "dcnt -tls=true"
	flag.StringVar(&p.dcntLocations, "selfaddr", "", "comma separated IPAddresses and DNS names of this dcnt to use when creating a cert file")
	...
	flag.BoolVar(&p.Fast, "fast", true, "If true, dcnt will fast-boot from a file.")
