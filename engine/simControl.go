...
import (
	...
	"github.com/DCNT-developer/dcnt/controlPanel"
	elections2 "github.com/DCNT-developer/dcnt/elections"
		...
		if err == nil && v >= 0 && v < len(fnodes) && fnodes[ListenTo].State != nil {
			...
			case 'i' == b[0]:
				...
				if len(b) > 1 && (b[1] == 'H' || b[1] == 'h') {
					...
					os.Stderr.WriteString("              identities that another instance of dcnt has added to their identity pool.\n")
					os.Stderr.WriteString("              Each dcnt instance has its own identity pool it can use (t), but everyone\n")
					os.Stderr.WriteString("              will share the identities in the identity stack. This stack is fixed and will\n")
					os.Stderr.WriteString("              be the same each time dcnt launches. It may grow in the future. Used for testing\n")
